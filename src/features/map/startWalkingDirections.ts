...
export async function startWalkingDirections(destination: Place): Promise<void> {
  ...
  if (!userLocation) {
    ...
    // Dialog.alert (non window.alert): nella WebView nativa di Capacitor,
    // alert()/confirm()/prompt() del browser possono bloccare l'app invece
    // di mostrare un dialogo — bug reale riscontrato su dispositivo reale.
    await Dialog.alert({ message: 'Please enable "Use my location" first, so we can calculate your route.' });
    return;
  }
  ...
  try {
    ...
    await Dialog.alert({ message: 'Could not calculate a walking route right now. Please try again.' });
