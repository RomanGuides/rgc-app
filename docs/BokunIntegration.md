...
- **Partially verified, real-card test still needed on both platforms to close out:** on a real Android device, the full flow now reaches the card-entry step (confirmed 2026-08-05). Still open: completing an actual booking with a European card (3-D Secure is mandatory under EU rules) through to a successful charge, on Android and, once the iOS fix exists, on iOS — 3-D Secure/PayPal/Apple Pay may still hit the "destination refuses framing" case if Bokun's own internal iframe (or a bank's 3-D Secure page) sends `X-Frame-Options: DENY`; if so, the modal's load-timeout/error handling and "Open in browser" remain the genuine escape hatch, not a fallback for a failure that shouldn't happen.
