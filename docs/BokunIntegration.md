...
> **Status (2026-08-05): DECIDED for now, "Go to cart" bug fixed.** After comparing embedded-widget vs. full-native checkout (see Decision below), the team chose the embedded widget. A real bug blocking the flow past "Go to cart" was found and fixed the same day — the root cause was that the initial implementation used a raw `<iframe>` instead of Bokun's actual official embed mechanism (see Decision below for the full story). §1–§9 below remain as a fully-researched reference for the native path, kept intentionally rather than deleted — it's the natural next step if/when the business case for it changes. `BookingWidgetModal.tsx` (§Decision) is the implemented piece; everything else in this document is still just a design for later.

## Decision (2026-08-05): embedded Bokun widget, native checkout deferred

**Chosen: Option 3 — Bokun's hosted widget, rendered inside the app using Bokun's own official embed mechanism, no browser chrome.** Implemented in `BookingWidgetModal.tsx`, replacing the external-browser link in `ExperiencesScreen.tsx`.
...
- **No Capacitor plugin needed for rendering.** This is different from `@capacitor/browser` (a separate native browser view with its own, unavoidable chrome/toolbar) — the whole app already runs inside one Capacitor WebView, so Bokun's widget renders inside the page that's already there. `@capacitor/app` was added, but only for one thing: intercepting the Android hardware back button so it closes the modal instead of navigating the widget's own history.
- **Not a plain `<iframe>` — this was tried first and is a documented dead end (see "Go to cart" below).** The actual implementation loads Bokun's own `BokunWidgetsLoader.js` (parametrized with the booking channel's UUID, extracted from `experience.bookingUrl`) once per app session, then renders a placeholder `<div class="bokunWidget" data-src={experience.bookingUrl}>`. Bokun's own script — not ours — detects that div (via a `MutationObserver` it installs on `document.body`) and takes over its contents entirely: creating its own internal iframe, wiring up cart/checkout, and marking the element `data-bokun-widget-loaded="true"` once initialized. This is Bokun's documented, official embed mechanism (visible in the Bokun admin under Sales tools → Booking widgets → generate embed code) — every widget type (Button, Product page, Product list, ...) generates the same `<script src=".../BokunWidgetsLoader.js?bookingChannelUUID=...">` + `<div class="bokunWidget" data-src="...">` pair, never a raw iframe.
- Bokun's widget URLs (`widgets.bokun.io/online-sales/...`, already stored per-experience in `experiences.json`'s `bookingUrl`) are exactly the value the official embed code puts in `data-src` for a single-experience widget — confirmed by comparing against a real embed code generated from the Bokun admin for a second, unrelated experience (same channel UUID, same URL shape, different experience id).
- `allow="payment"` is still set on the fallback iframe used for the native "new window" relay below (see next bullet) — without it, the Permissions Policy some browsers/WebViews apply by default can silently block the Payment Request API (used by Apple Pay/Google Pay and often by Stripe.js) inside a cross-origin iframe. We don't control the `allow` attribute of the iframe Bokun's own bundle creates internally inside the `.bokunWidget` div — that's entirely on their side now.
- **A real limit on the "is this page trustworthy" question §0 raised about the widget approach:** the parent page cannot read a cross-origin iframe's current URL — the browser's same-origin policy blocks it outright, not something to work around. Since we now supply `data-src` ourselves, the initial hostname shown in `BookingWidgetModal.tsx`'s trust row (padlock + hostname) is known synchronously and doesn't depend on reading anything cross-origin; it's only *further*, in-flow navigation inside Bokun's own internal iframe we can't necessarily observe. The component reads that internal iframe's `src` attribute directly when it appears (attribute access on an element in our own DOM tree is not restricted, only reaching into its cross-origin *document* is) as a best-effort improvement, falling back to the known-good initial host otherwise.
- **"Go to cart" bug — real root cause, found by live on-device debugging, not a theory (both earlier fixes below were wrong guesses).** With the original raw-`<iframe src={bookingUrl}>` implementation, browsing the experience and choosing a date/time worked, but tapping "Go to cart" did *nothing* — confirmed step by step with Chrome DevTools remote-debugged against the real device (`chrome://inspect`): the tap landed on exactly the right DOM element (`document.elementFromPoint` at the button's true screen coordinates returned the `GoToCartButton` span itself, ruling out an invisible overlay), a direct `.click()` via the JS console produced the same *zero* effect as the real tap (ruling out any WebView touch-to-click translation issue), there was no console error, no `postMessage` to the parent, and `document.body.innerText.length`/`location.href` were byte-for-byte identical before and after. Two earlier hypotheses were tried and diagnostically disproven via `Log.d` instrumentation before the real cause was found (both left in place below as documented dead ends, and as still-useful general hardening):
  1. *Disproven:* Capacitor's default WebChromeClient not implementing `onCreateWindow` (silently drops `target="_blank"`/`window.open()`). `RomanGuidesWebChromeClient` was added to relay such requests as a `romanguides:newwindow` DOM event — but logging proved `onCreateWindow` never fired for this button at all.
  2. *Disproven:* Capacitor's `shouldOverrideUrlLoading` (`Bridge.launchIntent`) treating nested-iframe navigation as an external-Intent candidate. `RomanGuidesWebViewClient` was added to let non-main-frame navigation proceed normally — but logging proved this override never fired for this button either.
  3. *Actual cause, found afterwards:* a raw iframe isn't Bokun's real embed mechanism (see the bullet above). Their real embed's script bundle is what handles "Go to cart" internally (very likely by rendering a cart/checkout overlay that requires their own JS to be present and initialized) — a bare `<iframe src="...">` never gives their code anything to hook into for that step, so the click handler (which does exist and does fire, confirmed with the `.click()` test above) silently has nothing to do. Fixed by switching to the loader-script + placeholder-div embed described above — verified working end-to-end on the same physical device: "Go to cart" now opens the real cart, and the flow proceeds through participant/contact details all the way to the card-entry step.
- `RomanGuidesWebChromeClient`/`RomanGuidesWebViewClient` are kept even though neither was the actual fix — they're still correct, general-purpose hardening for a real, separate risk: a payment step (3-D Secure, PayPal, Apple Pay) genuinely opening a popup from *inside* Bokun's own widget iframe. `BookingWidgetModal.tsx` still listens for the `romanguides:newwindow` relay and shows a full-screen fallback iframe for that case.
- **iOS has no equivalent fix yet, and almost certainly needs one** for that same popup case. `WKWebView` has the same category of gap — `WKUIDelegate`'s `createWebViewWith(configuration:for:windowFeatures:)` isn't implemented by Capacitor's default iOS bridge either. Not fixed in this pass: this whole project has been Android-only (no Mac/Xcode available in this environment) — untestable and therefore unwritten rather than shipped unverified. Tracked in `ROADMAP.md`. Note this no longer affects the core "Go to cart"/add-to-cart flow (that's fixed by the embed-mechanism change, which is platform-agnostic) — it only matters for the payment-popup edge case.
- **Verified on Android, iOS still open:** a real European-card booking on a physical Android device (2026-08-05) went end-to-end through 3-D Secure to a successful charge, with no framing issues (`X-Frame-Options` never blocked Bokun's internal iframe or the bank's 3-D Secure page). Same test still needs doing on iOS once the `WKWebView` equivalent of `RomanGuidesWebChromeClient`/`RomanGuidesWebViewClient` exists (tracked in `ROADMAP.md`) — no reason to expect a different result, but unverified. If a future case does hit "destination refuses framing," the modal's load-timeout/error handling and "Open in browser" remain the genuine escape hatch, not a fallback for a failure that shouldn't happen.
