...
## Empty and error states (redesign v1 addendum)

`RomeSheet.tsx`, `SearchScreen.tsx`, `PlaceScreen.tsx`, and `MapView.tsx`/`MapScreen.tsx` implement the states from the "Empty and Error States" design addendum:

- **Search, no match** (`SearchScreen.tsx`) — when a query matches nothing, up to 3 suggestion pills are computed from the bundle (never a network call): the category of the place whose *name* is lexically nearest the query (Levenshtein distance, `src/utils/levenshtein.ts`, skipped if the nearest distance exceeds 5), the area of the place nearest the user's location (or, without a location, the area with the most places inside the map's current viewport — see `mapBounds` below), and that first place's own area if it differs from the second pill. Tapping a pill replaces the query text and re-runs the existing filter, it isn't a separate filter mechanism.
- **Filters, no results** (`RomeSheet.tsx`) — reachable today only by deselecting every category in the filter popover (every category has at least one place, so that's the sole way `visiblePlaces` goes to zero). Offers two distinct actions: "Show nearest places" (re-enables just the category/categories of the 1-2 nearest places — our adapted equivalent of "widen", since the "Around Me" radius this design assumes was removed in Phase 3) and "Clear filters" (re-enables all categories via the store's `setActiveCategories`).
- **Location refused** (`RomeSheet.tsx`) — when `useGeolocation` status is `'denied'`, "Nearest to you" is replaced (not emptied) by a by-neighbourhood grouping of `visiblePlaces`, using each place's canonical `area` (see `docs/DataModel.md`), sorted by place count descending then alphabetically; areas with a single place still get their own group.
- **Offline, map tiles fail** (`MapView.tsx`, `MapScreen.tsx`, `OfflineBanner.tsx`) — `useOnlineStatus.ts` wraps `navigator.onLine` + the `online`/`offline` events. Offline, `MapScreen` renders a non-dismissable, no-retry banner (disappears on its own when back online) and forces `RomeSheet` to its `full` detent (`forceFullDetent` prop) so the list — not an increasingly stale map — carries the screen. `MapView`'s container has a permanent `#F3EFEB` background so a tile that fails to load (offline or otherwise) leaves this color, never a grey checkerboard.
- **Place with missing data** (`PlaceScreen.tsx`) — editorial sections and the facts row already collapsed cleanly when a field was absent (Phase 4). The one gap this addendum closed: a *present but broken* `imageUrl` (network failure, 404) now falls back to the category color exactly like a missing one — detected by preloading the image via a plain `Image()` object, since a failed CSS `background: url(...)` doesn't emit anything a component can observe directly.

`mapBounds` (`usePlacesStore.ts`, `[west, south, east, north]`) is written by `MapView.tsx` on `moveend`/`zoomend` and read only by `SearchScreen.tsx`'s no-match pills — the map itself never reads it back.

The 27 canonical place `area` values these states rely on are validated at build time — see `docs/DataModel.md` and `scripts/check-place-areas.mjs`.
