...
**On Windows, `cap sync` regenerates `ios/App/CapApp-SPM/Package.swift` with backslash paths that Swift cannot compile.** Every `npm run cap:sync` rewrites that file (it is generated, not hand-maintained) and on Windows writes the plugin paths using the host OS separator: `.package(name: "CapacitorApp", path: "..\..\..\node_modules\@capacitor\app")`. In Swift a backslash starts an escape sequence, so `\.` and `\@` are invalid and the file would fail to compile on a Mac. This has no effect on the Android or web builds, and iOS has never been built for this project (no Mac available — see the current-state section above), so it is latent rather than active. Two practical consequences: (1) the file shows up as an unexplained modification in `git status` after any `cap sync` run on Windows — it is generated noise, not someone's edit; (2) **do not commit the Windows-generated version.** Whoever first builds for iOS should run `cap sync` on the Mac itself, which regenerates the same file with forward slashes, and commit that. Same class of environment trap as the JDK 21 requirement above: the toolchain works fine on the machine you are on, and produces output that only breaks on the platform you are not on yet.
